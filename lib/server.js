...
import { PageMyPosts } from '../pages/PageMyPosts.js';
...
server.httpServer = http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    if (isAPI) {
      ...
      const APIRouteList = dataForHandlers.user.isLoggedIn
        ? server.protectedAPI
        : server.publicAPI;
    ...
    if (isPage) {
      ...
      const routeList = dataForHandlers.user.isLoggedIn
      ...
      if (level2) {
        pageClass = server.secondLevelRoutes[level1]
          ? server.secondLevelRoutes[level1]
          : routeList['404'];
      }
...
server.extendCookies = async tokenID => {
  ...
  const [updateErr, updateMsg] = await file.update(
    'token',
    tokenID + '.json',
    obj
  );
